...
class ProjectsCacheImpl @Inject constructor(
    ...
    override fun getProjects(): Observable<List<ProjectEntity>> {
        return projectsDatabase.cachedProjectsDao().getProjects().toObservable().map {  it.map{mapper.mapFromCached(it)}}
    }
    ...
    override fun getBookmarkedProjects(): Observable<List<ProjectEntity>> {
        return projectsDatabase.cachedProjectsDao().getBookmarkedProjects().toObservable().map { it.map{mapper.mapFromCached(it)} }
    }
    ...
    override fun setLastCached(lastCacheTime: Long): Completable {
         return Completable.defer {
             projectsDatabase.configDao().insertConfig(Config(lastCacheTime))
             Completable.complete()
         }
    }
    ...
    override fun isProjectCacheExpired(): Single<Boolean> {
        ...
        return projectsDatabase.configDao().getConfig().single(Config(0)).map { currentTime - it.lastCacheTime > expirationTime }
    }
